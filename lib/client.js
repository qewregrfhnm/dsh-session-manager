		...
		*/
		...
		function SessionManager({ useSessions, useWorkspaces, api, sessions, workspaceActions, close }) {
			...
			const handleMoveToWorkspace = (0, react.useCallback)(async (sessionId, workspaceId) => {
				...
				try {
					...
					if (!response.ok || data.ok !== true) {
						...
						showNotice({
							kind: "error",
							text: strings.moveFailed(data.error ?? `HTTP ${response.status}`) + suffix + detail
						});
						return;
					}
					...
					showNotice({
						kind: "error",
						text: strings.moveFailed("network")
					});
				} finally {
					setBusyId(null);
				}
			...
			const renderRow = (session, isArchived) => {
				...
				}), moveOpenId === session.id && (0, react.createElement)("div", { className: "dsh-delete-session__more-menu" }, ...(() => {
